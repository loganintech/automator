...
pub struct Task<AT, TT, AE, TE, AA> {
    a: Pin<Box<dyn Action<AT, AE, AA>>>,
    t: Pin<Box<dyn Trigger<TT, TE>>>,
...
impl<'a, AT, TT: Into<AA>, AE, TE, AA> Task<AT, TT, AE, TE, AA> {
    pub async fn check(&'a mut self) -> Result<(), ErrorSource<AE, TE>> {
        let checked = self.t.check();
        let mut checked = checked.await;
        Ok(())
...
pub struct TaskBuilder<AT, TT, AE, TE, AA> {
    a: Option<Pin<Box<dyn Action<AT, AE, AA>>>>,
    t: Option<Pin<Box<dyn Trigger<TT, TE>>>>,
    conv: Option<Box<dyn Fn(TT) -> AA>>,
}
